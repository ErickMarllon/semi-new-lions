...
export const Footer = () => {
  return (
    <motion.footer
      className="bg-background border-t border-border/50 min-w-full"
      initial="hidden"
      whileInView="show"
    ...
    >
      <motion.div className="mx-auto px-4" variants={containerVariants}>
        <motion.div
          className="py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16"
          variants={containerVariants}
        >
      ...
      >
        ...
        <form className="flex gap-2">
          <input
            ...
            className="flex-1 md:min-w-md bg-secondary border border-border/50 rounded-lg px-4 py-2 text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary"
          />
          <Button
            variant={"gold"}
            type="button"
            className="h-full"
            onClick={() => toast.error("Funcionalidade ainda não implementada")}
          >
            <Iconify
      ...
      <motion.div
        className="py-6 w-full mt-10 border-t border-border/50 flex flex-col md:flex-row items-center justify-between gap-4"
        variants={itemVariants}
      >
